...
#Read and unnest rice data

rice_data<- getONAdatasets(username = username, password = password, form_id = "752109")
#unnest

rice_data_unnested <- unnest_variable(rice_data, "plot")
rice_data_unnested1 <- unnest_variable(rice_data_unnested, "plot_plot/plant1")
rice_data_unnested2 <- unnest_variable(rice_data_unnested1, "plot_plot/plant2")
...
PD_data<- rice_data_unnested2 %>% 
  dplyr::select(FDID2,TLID2,`plot_plot/pestDiseases`,grep(paste(".*severity.*|.*incidence.*", collapse = "|"), names(rice_data_unnested2),value = T)) %>% 
  rename_with(
    ~stringr::str_replace_all(.x, "plot_plot/PD_parameters[:digit:]/", "")) %>%  #shorten variable names
