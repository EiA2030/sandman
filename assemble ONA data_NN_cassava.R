...
# 2)  plant stand
plantStand<-c("nrPlants",	"plotWidth","firstLevelBranchesNr",
              "plotLength","nrPlants_branching", "mainStemsNr")
...
tuberYield<- c("tuberizedRootsNr","tuberizedSmallRootsNr","tuberizedMarketableRootsNr","tuberizedRootsFW","tuberizedDiseasedRootsFW",
               "tuberizedSmallRootsFW",	"tuberizedMarketableRootsFW",	"tuberizedRootsFWss",	"tuberizedRootsPSID",
               "tuberizedDiseasedRootsFWss",	"tuberizedDiseasedRootsPSID",	"tuberizedSmallRootsFWss",	"tuberizedSmallRootsPSID",
               "tuberizedMarketableRootsFWss",	"tuberizedMarketableRootsPSID")

# Check if variables exist in the dataframe
matching_elements <- names(data_unnested)[grepl(paste(tuberYield, collapse = "|"), names(data_unnested))] 
matching_elements= gsub("plot_plot/rootYield[0-9]_parameters/","",matching_elements)
...
tuberYield_data<- data_unnested %>% 
  ...
  rename_with(
    ~stringr::str_replace_all(.x, c("plot_plot/rootYield[0-9]_parameters/"), "")) #shorten variable names
...
dplyr::mutate_at(c("tuberizedRootsNr","tuberizedSmallRootsNr","tuberizedMarketableRootsNr","tuberizedRootsFW",
                   "tuberizedDiseasedRootsFW","tuberizedSmallRootsFW",	"tuberizedMarketableRootsFW",	
                   "tuberizedRootsFWss","tuberizedDiseasedRootsFWss",	"tuberizedSmallRootsFWss",
                   "tuberizedMarketableRootsFWss"), as.numeric) %>% 
  group_by(FDID2, TLID2,`plot_plot/POID2`,`plot_plot/POID2_label`) %>%
  summarise(across(where(is.numeric), list(mean = ~mean(., na.rm = TRUE))))%>% 
  rename_with(~gsub("_mean", "", .), contains("mean"))
