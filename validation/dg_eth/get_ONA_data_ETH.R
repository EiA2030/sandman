    ...
    for(i in 1:nL){
      ...
      if(nrow(mi) > 0){
        ...
        mi <- mi %>%
          ...
          dplyr::select(-contains("trialRating")) %>% # Removes the trialRating group since we are only focusing on the harvest group (ASSUMPTION!!!)
          #dplyr::rename_all(basename) #drop group names 
          dplyr::rename_all(stringr::str_extract, "\\w+$") # Renames the columns using only the name after the group ("/")
