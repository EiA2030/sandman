...
Plant_stand_data<- data_unnested %>% 
  dplyr::select(FDID2,TLID2,`plot_plot/POID2`,`plot_plot/POID2_label`,today,plantStand) %>% 
  rename_with(
...
# get average  values (Use ID-RSPORW622018777139 to confirm any difference between sample data average and script 
Plant_stand_data_summ<- Plant_stand_data %>% 
  dplyr::mutate_at(c("nrPlants","plotLength","plotWidth"), as.numeric) %>% 
...
tuberYield<- c("plot_plot/tuberYield1_parameters/tubersNr", "plot_plot/tuberYield2_parameters/tubersFW",            
...
tuberYield_data<- data_unnested %>% 
  dplyr::select(FDID2,TLID2,`plot_plot/POID2`,`plot_plot/POID2_label`,tuberYield) %>% 
  rename_with(
    ~stringr::str_replace_all(.x, c("plot_plot/tuberYield3_parameters/|plot_plot/tuberYield2_parameters/|plot_plot/tuberYield1_parameters/"), "")) #shorten variable names
...
# get average  values since some plots have multiple harvest measurements (Use ID-RSPORW002658739184 to confirm if there are any difference between sample data average and script 
tuberYield_data_summ<-tuberYield_data %>% 
dplyr::mutate_at(c("tubersNr","tubersFW","tubersFWss","tubersSmallNr","tubersSmallFW","tubersDiseasedNr",    
                     "tubersDiseasedFW","tubersMarketableNr","tubersMarketableFW", "tubersSmallFWss",
                     "tubersDiseasedFWss", "tubersMarketableFWss"), as.numeric) %>% 
  group_by(FDID2, TLID2,`plot_plot/POID2`,`plot_plot/POID2_label`) %>%
  summarise(across(where(is.numeric), list(mean = ~mean(., na.rm = TRUE))))%>% 
  rename_with(~gsub("_mean", "", .), contains("mean"))
