...
tuberYield<- c("grainsFWss","grainsFW","grainsMC")
...
dplyr::mutate_at(c("grainsFWss","grainsFW","grainsMC"), as.numeric) %>% 
  group_by(FDID2, TLID2,`plot_plot/POID2`,`plot_plot/POID2_label`) %>%
  summarise(across(where(is.numeric), list(mean = ~mean(., na.rm = TRUE))))%>% 
  rename_with(~gsub("_mean", "", .), contains("mean"))
...
writexl::write_xlsx(combined_data,"./data/summary_Beans_plot_data.xlsx")
