...
data_unnested<- data_unnested %>% 
  unnest(`plot_plot/PD`,keep_empty = TRUE,.drop = TRUE, names_sep = "_") 


library(dplyr)
library(purrr)
cc=data_unnested %>%
  mutate(n = pmax(lengths(`plot_plot/PD`), lengths(`plot_plot/plant1`),lengths(`plot_plot/plant2`))) %>% 
  mutate_at(vars(`plot_plot/PD`,`plot_plot/plant1`,`plot_plot/plant2`), ~ 
              map2(., n, ~ `length<-`(.x, .y,.z))) %>% 
  select(-n) %>%
  unnest(cols = c(`plot_plot/PD`, `plot_plot/plant1`,`plot_plot/plant1`))
...
cc=data_unnested %>%
  mutate(`plot_plot/plant2` = lapply(`plot_plot/plant2`, as.character),
         `plot_plot/plant1` = lapply(`plot_plot/plant1`, as.character)) %>% 
  mutate_at(.vars = vars(`plot_plot/plant1`,`plot_plot/plant2`), as.list) %>%
  unnest(`plot_plot/plant1`,`plot_plot/plant2`,keep_empty = TRUE,.drop = TRUE, names_sep = "_")

cc <- data_unnested %>%
  select(-c(`_attachments`,`_tags`,`_notes`)) %>% 

  unnest(c(`plot_plot/PD`,`plot_plot/plant1`,`plot_plot/plant1`), keep_empty = TRUE,.drop = TRUE, names_sep = "_") 

df <- data_unnested %>%
  select(-c(`_attachments`,`_tags`,`_notes`,`_geolocation`)) %>% 
  mutate(across(where(is.list), ~unnest(.,keep_empty = TRUE, names_sep = "_")))
...
# 2)  plant stand
# drop generic variables to capture plot level variable 
#data_unnested=select(data_unnested,-c(tillerNumber,plantHeight))

plantStand<-c("nrPlants",	"plotWidthPlantStand","plotLengthPlantStand","plantHeightNr", 
              "tillerNumberNr","tillerNumber","plantHeight")
...
matching_elements= gsub("plot_plot/plantStand_parameters/|plot_plot/plant[0-9]/tillerNumber_parameters/|plot_plot/plant[0-9]/plantHeight_parameters/|plot_plot/",
                        "",matching_elements)
...
Plant_stand_data<- data_unnested %>% 
  ...
  rename_with(
    ~stringr::str_replace_all(.x, "plot_plot/plantStand_parameters/", "")) %>%  #shorten variable names
...
Plant_stand_data_summ<- Plant_stand_data %>% 
  dplyr::mutate_at(c("nrPlants",	"plotWidthPlantStand","plotLengthPlantStand","plot_plot/plantHeightNr", 
                     "plot_plot/tillerNumberNr","tillerNumber","plantHeight"), as.numeric) %>% 
...
tuberYield<- c("grainsFW","paniclesFW","plotWidthGrainYield","plotLengthGrainYield","totalGrainsFW","plotLengthBiomass",
               "plotWidthBiomass","grainsMC","biomassFW","biomassFWss","biomassPSID")
...
dplyr::mutate_at(c("plotWidthGrainYield","plotLengthGrainYield","grainsFW","paniclesFW",
                   "totalGrainsFW","grainsMC","plotLengthBiomass","biomassFWss",
                   "plotWidthBiomass","biomassFW"), as.numeric) %>% 
  group_by(FDID2, TLID2,`plot_plot/POID2`,`plot_plot/POID2_label`) %>%
  summarise(across(where(is.numeric), list(mean = ~mean(., na.rm = TRUE))))%>% 
  rename_with(~gsub("_mean", "", .), contains("mean"))
